...
def get_reconstructed_scene(outdir, model, device, silent, image_size, filelist, schedule, niter, min_conf_thr,
    ...
    print("Check im_poses: ", scene.im_poses)
    ...
    if redo_global_alignment:
        ...
        im_poses_4by4 = torch.stack(cams2world_scaled)
        unnormalized_im_focals = to_numpy(scene.get_focals()) # [None] * len(imgs) # to_numpy(scene.im_focals).tolist()
        ...
        global_alignment_optimization(output, imgs, device, silent, niter, schedule, lr=0.01, init='known_pts3d', \
                                      scene=None, pts3d=pts3d_scaled, im_focals=unnormalized_im_focals, im_poses=im_poses_4by4)
        # import pdb; pdb.set_trace()
