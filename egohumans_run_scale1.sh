...
if [ $use_gt_focal = True ]; then
    ...
    cmd="CUDA_VISIBLE_DEVICES=0 python hongsuk_egohumans_align_dust3r_hmr2hamer_scale1.py --sel-big-seqs $big_seq --sel_small_seq_range $small_seq --use_gt_focal"
else
    cmd="CUDA_VISIBLE_DEVICES=0 python hongsuk_egohumans_align_dust3r_hmr2hamer_scale1.py --sel-big-seqs $big_seq --sel_small_seq_range $small_seq"
fi
